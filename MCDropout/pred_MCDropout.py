...
parser = argparse.ArgumentParser(description='Sample Bayesian Neural Net with MC Dropout Variational Inference')
...
parser.add_argument('--nb_samples', type=int, default=50, help='Number of samples for each datapoint. Default: 50.')
...
parser.add_argument('--models_dir', type=str, nargs='?', action='store', default='MCdrop_models',
                    help='Where to get learnt models from. Default: \'MCdrop_models\'.')
...
# Load model
net = MC_drop_net(channels_in=channels_in, side_in=input_size, cuda=use_cuda, device=device, classes=num_classes, batch_size=args.batch_size,
                  n_hid=100, pdrop=pdrop, data=data, model=args.model)
net.load(models_dir+'/'+args.model+'_'+data+'_pdrop'+str(pdrop)+'_seed'+str(args.seed)+'.pt')
...
bin_confidences, bin_accuracies, ece = plot_calibration_diagram(flat_confs, flat_preds, np_labels, num_bins , args.model, args.seed, args.id, data, args.ood_data, Nsamples, pdrop)
...
kl_div = plot_histogram(id_array, ood_array, args.model, args.measure, args.seed, args.id, data, args.ood_data, Nsamples, pdrop)
...
plot_roc(tpr, fpr, args.model, data, args.ood_data, auroc, UA_values, args.seed, args.id, pdrop)

# plot accuracy-rejection curves
fractions, accuracies, ideal, filtered_imgs, filtered_uncs, filtered_lbls = calc_rejection_curve(all_labels, flat_preds, id_array, ood_array, device, all_images, all_ood_images)
plot_accrej_curve(fractions, accuracies, ideal, args.model, data, args.ood_data, args.seed, args.id, pdrop)
...
if args.save_txt:
    ...
    results_file = file_path+'/%s_%s_%s_%s_%s_id%s.txt'%(args.model, args.measure, data, pdrop, str(args.seed), str(args.id))
...
if args.final_model:
    plot_dir =  "../Integration/MCDropout"
