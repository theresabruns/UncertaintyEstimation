...
def plot_histogram(id_array, ood_array, model, measure, seed, id, data, ood_data, Nsamples, pdrop):
    ...
    if ood_data == 'usecase':
        titlename = data+' - Model Seed: '+str(seed)+' , Samples: '+str(Nsamples) +' , Pdrop: '+str(pdrop) +' , KL-div: '+str(round(kl_div, 2))
        filename = hist_path+'/MCDO_'+model+'_'+measure+'_'+data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
    else: 
        titlename = data+' vs. '+ood_data+' - Model Seed: '+str(seed)+' , Samples: '+str(Nsamples) +' , Pdrop: '+str(pdrop) +' , KL-div: '+str(round(kl_div, 2))
        filename = hist_path+'/MCDO_'+model+'_'+measure+'_'+data+'_'+ood_data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
...
def plot_roc(tpr, fpr, model, data, ood_data, auroc, UA_values, seed, id, pdrop):
    ...
    if ood_data == 'usecase':
        titlename = 'ROC-curve MCDropout - '+ood_data+' , Pdrop: '+str(pdrop)+' - AUROC: '+str(round(auroc, 4)) \
                    +' - UAcc: '+str(round(best_ua, 2)) +' - Thresh: '+str(round(best_thresh, 2))
        filename = roc_path+'/MCDO_roc_'+model+'_'+data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
    else: 
        titlename = 'ROC-curve MCDropout - '+data+' vs.'+ood_data+' , Pdrop: '+str(pdrop)+' - AUROC: '+str(round(auroc, 4)) \
                    +' - UAcc: '+str(round(best_ua, 2)) +' - Thresh: '+str(round(best_thresh, 2))
        filename = roc_path+'/MCDO_roc_'+model+'_'+data+'_'+ood_data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
...
def plot_calibration_diagram(confidences, predictions, all_labels, num_bins, model, seed, id, data, ood_data, Nsamples, pdrop):
    ...
    plt.style.use('seaborn-v0_8')
    plt.plot(bin_confidences, bin_accuracies, label=str(pdrop)+'-MCDropout', color = 'tab:blue', linestyle="-")
    ...
    if ood_data == 'usecase':
        titlename = 'Calibration MCDropout - '+ood_data+' - ECE: '+str(round(ece, 2))\
                    +' - Avg-Acc: '+str(round(avg_acc, 2))+' - Avg-Conf: '+str(round(avg_conf, 2))
        filename = cal_path+'/MCDOcal_'+model+'_'+data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
    else: 
        titlename = 'Calibration MCDropout - '+data+' vs.'+ood_data+' - ECE: '+str(round(ece, 2))\
                    +' - Avg-Acc: '+str(round(avg_acc, 2))+' - Avg-Conf: '+str(round(avg_conf, 2))
        filename = cal_path+'/MCDOcal_'+model+'_'+data+'_'+ood_data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
...
def plot_accrej_curve(fractions, accuracies, ideal, model, data, ood_data, seed, id, pdrop):
    # plot and save accuracy-rejection curve
    plt.style.use('seaborn-v0_8')
    plt.plot(fractions, accuracies, label=str(pdrop)+'-MCDropout', color = 'tab:orange', linestyle="-")
    ...
    if ood_data == 'usecase':
        titlename = 'Acc-Rej-curve MCDropout - '+ood_data
        filename = accrej_path+'/MCDOaccrej_'+model+'_'+data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
    else: 
        titlename = 'Acc-Rej-curve MCDropout - '+data+' vs.'+ood_data
        filename = accrej_path+'/MCDOaccrej_'+model+'_'+data+'_'+ood_data+'_pdrop'+str(pdrop)+'_seed'+str(seed)+'_id'+str(id)+'.png'
