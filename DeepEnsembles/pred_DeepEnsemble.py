...
parser = argparse.ArgumentParser(description='Sample Bayesian Neural Network with a Deep Ensemble')
...
parser.add_argument('--measure', type=str, default='entropy', choices=['entropy', 'mutualinfo', 'conf'], help='Uncertainty measure')
parser.add_argument('--data', type=str, default='usecase', choices=['svhn', 'mnist', 'usecase'], help='ID data used for model training')
parser.add_argument('--ood_data', type=str, default='usecase', choices=['cifar10', 'notmnist', 'fashionmnist', 'usecase'], help='OOD data to test MNIST model on')
parser.add_argument('--usecase', type=str, default='RR', choices=['RR', 'Emblem'], 
                        help='Which use case data the model was trained on')
parser.add_argument('--members', type=int, default=10, help='# of Ensemble members')
...
parser.add_argument('--models_dir', type=str, nargs='?', action='store', default='Ensemble_models',
                    help='Where to get learnt models from. Default: \'Ensemble_models\'.')
...
if args.ood_data == 'usecase' and args.data == 'usecase':
    ...
    if args.ood_data == "fashionmnist":
        ...
        _, _, ood_loader, _, _ = get_CIFAR10loaders(datapath, batch_size=args.batch_size)
elif args.data == 'svhn':
...
net = Ens_net(channels_in=channels_in, side_in=input_size, cuda=use_cuda, device=device, 
            classes=num_classes, batch_size=args.batch_size, n_hid=100, data=data, model=args.model)
...
for j, (images, labels) in enumerate(tqdm(test_loader)):
    ...
    # get results from average of samples
    batch_error, batch_nll, batch_brier, probs, mean_entropy = net.get_sample_scores(images, labels, num_classes, modeldir=models_dir, model=args.model, seed=args.seed, data=data, members=args.members)
...
for j, (ood_images, ood_labels) in enumerate(tqdm(ood_loader)):
    ...
    _, _, _, ood_probs, ood_mean_entropy = net.get_sample_scores(ood_images, ood_labels, num_classes, modeldir=models_dir, model=args.model, seed=args.seed, data=data, members=args.members)
...
bin_confidences, bin_accuracies, ece = plot_calibration_diagram(flat_confs, flat_preds, np_labels, num_bins , args.model, args.seed, args.id, data, num_members)
...
kl_div = plot_histogram(id_array, ood_array, args.model, args.measure, args.seed, args.id, data, num_members)
...
plot_roc(tpr, fpr, args.model, data, auroc, UA_values, args.seed, args.id, num_members, args.measure)
...
plot_accrej_curve(fractions, accuracies, ideal, args.model, data, args.seed, args.id, num_members, args.measure)
...
if args.save_txt:
    ...
    results_file = file_path+'/%s_%s_%s_%s_%s_id%s.txt'%(args.members, args.model, args.measure, data, str(args.seed), str(args.id))
...
if args.final_model:
    plot_dir =  "../Integration/DeepEnsembles"
    if not os.path.exists(plot_dir):
        os.makedirs(plot_dir)
    plot_file = plot_dir+'/%s_%s_%s_%s_%s.npz'%(args.model, args.members, args.measure, data, str(args.seed))
